from Encryption import encrypt, decrypt, make_code, WeakCodeError  # My program
import datetime
...
from Open_drive import save_to_drive, open_file  # My program
from ConvergeEntries import write_new_file, SDE_files  # My program
import pickle
import traceback
import Change_pass_user  # My program
...
Application_Name = "Journal SDE 1.3.0"
...
def caldates():
    ...
    for file in files:
        try:
            day = datetime.datetime(int(file[0]), int(file[1]), int(file[2]))
            dates.append(day)
        except:
            continue
...
def save_data(file_date, text_box):
    print('---| Saving Data for :', file_date, " |---")
...
def set_left_frame(root, app, text_box):
    global current_date, cal_variable
    day_var = tk.StringVar()
    # Only the variable change whenever needed
    current_date = (today_date_num('d'))+"-" + \
        (today_date_num('m'))+"-"+(today_date_num("y"))
    ...
    label = tk.Label(frame, textvariable=day_var)
    # comics----------------------------------------------------------
...
def make_options(frame):
    ...
    def change_password():
        ...
        def r_u_sure():
            # Before confirming, check if old password is correct and new one is valid
            ...
            if cont:
                ...
                confirm_but = ttk.Button(pop_up, text="Confirm",
                                         command=lambda: change_pass_ok(pop_up, confirm_but, new_code, old_code))
                confirm_but.pack()
                preset_window(pop_up)
                pop_up.mainloop()
        ...
        def change_pass_ok(pop_up, confirm_but, new_code, old_code):
            ...
            for file_completed_number, file_name in Change_pass_user.change_password(new_code, old_code):
    ...
    texts = [
        ...
        'Export entries as text file (Chosen Location)',
    ...
    for i, button in enumerate(buttons):
        ...
        t_text.config(bg=upper_bg, font=label_font, fg='white')
        t_text.place(x=275, y=y, w=500, h=50)


def set_options_page(frame):
    opt_font = ("Verdana bold", 30)
    bgframe = tk.Frame(frame, bg=upper_bg)
    bgframe.place(x=0, y=0, w=w, h=75)

    tk.Label(frame, text="Options Page",
             bg=upper_bg, font=opt_font,
             fg='white').place(x=245, y=10)
...
# main is for testing purposes and intro is for the
# actual and complete program
# main() # --> if you want only to test
intro() # --> if you want the complete program
